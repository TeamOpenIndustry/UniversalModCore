...
public class GLModelBuilder implements IModelBuilder {
    ...
    private final Buffers.FloatBuffer posIndexBuffer = new Buffers.FloatBuffer(1024);
    private final Buffers.FloatBuffer uvIndexBuffer = new Buffers.FloatBuffer(1024);
    private final Buffers.FloatBuffer normalIndexBuffer = new Buffers.FloatBuffer(1024);

    // 3 ints per vertex, 3 verts per triangle (posIdx, uvIdx, nrmIdx) x3
    private final Buffers.IntBuffer faceBuffer = new Buffers.IntBuffer(1024);
    private final Buffers.IntBuffer materialByVert = new Buffers.IntBuffer(1024);
    ...
    public int addIndexedVert(float x, float y, float z) {
        int index = posIndexBuffer.size() / 3;
        posIndexBuffer.add(x);
        posIndexBuffer.add(y);
        posIndexBuffer.add(z);
        return index;
    }

    @Override
    public int addIndexedUv(float u, float v) {
        int index = uvIndexBuffer.size() / 2;
        uvIndexBuffer.add(u);
        uvIndexBuffer.add(v);
        return index;
    }
    ...
    public int addIndexedNormal(float nx, float ny, float nz) {
        int index = normalIndexBuffer.size() / 3;
        normalIndexBuffer.add(nx);
        normalIndexBuffer.add(ny);
        normalIndexBuffer.add(nz);
        return index;
    }

    @Override
    public IFaceBuilder face() {
        return new GlFaceBuilder();
    }

    @Override
    public void finish() {
        // Texture repacking (per material) and normal generation happen here in the future
    }
    ...
    public Model build(VAOLayout layout) {
        ...
        for (int tri = 0; tri < triCount; tri++) {
            ...
            Vec3d pa = new Vec3d(posIndexBuffer.get(posIdx0 * 3), posIndexBuffer.get(posIdx0 * 3 + 1), posIndexBuffer.get(posIdx0 * 3 + 2));
            Vec3d pb = new Vec3d(posIndexBuffer.get(posIdx1 * 3), posIndexBuffer.get(posIdx1 * 3 + 1), posIndexBuffer.get(posIdx1 * 3 + 2));
            Vec3d pc = new Vec3d(posIndexBuffer.get(posIdx2 * 3), posIndexBuffer.get(posIdx2 * 3 + 1), posIndexBuffer.get(posIdx2 * 3 + 2));
            ...
            for (int k = 0; k < 3; k++) {
                ...
                if (hasUv) {
                    if (uvIdx >= 0) {
                        data[base + uvOff] = uvIndexBuffer.get(uvIdx * 2);
                        data[base + uvOff + 1] = uvIndexBuffer.get(uvIdx * 2 + 1);
                    }
                }

                if (hasColor) {
                    int matId = materialByVert.get(v);
                ...
                if (hasNormal) {
                    if (nrmIdx >= 0) {
                        data[base + nrmOff] = normalIndexBuffer.get(nrmIdx * 3);
                        data[base + nrmOff + 1] = normalIndexBuffer.get(nrmIdx * 3 + 1);
                        data[base + nrmOff + 2] = normalIndexBuffer.get(nrmIdx * 3 + 2);
                    } else {
        ...
        List<ModelGroup> groups = buildGroups(triCount);

        return new Model(layout, data, groups);
    }

    private List<ModelGroup> buildGroups(int triCount) {
        ...
        for (int i = 0; i < groupNames.size(); i++) {
            ...
            for (int j = start; j < end; j++) {
                int idx = faceBuffer.get(j * 3);
                points.add(new Vec3d(posIndexBuffer.get(idx * 3), posIndexBuffer.get(idx * 3 + 1), posIndexBuffer.get(idx * 3 + 2)));
            }
            ...
            result.add(new ModelGroup(groupNames.get(i), start, end, minN, maxN, normal));
        }
        return result;
    }
    ...
    public class GlFaceBuilder implements IFaceBuilder {
        ...
        @Override
        public IFaceBuilder vert(int posIdx, int uvIdx, int normalIdx) {
            buffer.add(posIdx);
            buffer.add(uvIdx);
        ...
        public void end() {
            ...
            if (vertCount == 3) {
                ...
                for (int i = 0; i < buffer.size(); i += 3) {
                    int p = buffer.get(i);
                    positions.add(new Vec3d(posIndexBuffer.get(p * 3), posIndexBuffer.get(p * 3 + 1), posIndexBuffer.get(p * 3 + 2)));
                }
            ...
            for (int i : indices) {
                ...
                materialByVert.add(currMaterial);
            }
        }
    }
}
