...
public final class PlaneBasis {
    ...
    public static PlaneBasis fromPlane(Plane plane) {
        ...
        Vec3d u = helper.crossProduct(n).normalize();// u = helper × normal
        Vec3d v = n.crossProduct(u).normalize();// v = normal × u
        return new PlaneBasis(u, v);
    }

    public PlaneBasis rotateCW() {
        return new PlaneBasis(v, u.scale(-1));
    }

    public PlaneBasis rotateCCW() {
        return new PlaneBasis(v.scale(-1), u);
    }

    public PlaneBasis flipU() {
        return new PlaneBasis(u.scale(-1), v);
    }

    public PlaneBasis flipV() {
        return new PlaneBasis(u, v.scale(-1));
    }

    public PlaneBasis flip() {
        return new PlaneBasis(u.scale(-1), v.scale(-1));
    }
}