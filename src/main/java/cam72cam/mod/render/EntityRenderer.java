...
public class EntityRenderer<T extends ModdedEntity> extends net.minecraft.client.renderer.entity.EntityRenderer<T> {
    ...
    private static void renderLargeEntities(RenderWorldLastEvent event) {
        ...
        net.minecraft.util.math.Vec3d vec3d = info.getProjectedView();
        double camX = vec3d.getX();
        double camY = vec3d.getY();
        double camZ = vec3d.getZ();

        ClippingHelperImpl camera = new ClippingHelperImpl(event.getMatrixStack().getLast().getMatrix(), event.getProjectionMatrix());
    ...
    public void render(T stock, float entityYaw, float partialTicks, MatrixStack p_225623_4_, IRenderTypeBuffer p_225623_5_, int i) {
        ...
        RenderContext.lastLightX = stock.getEntityWorld().getWorld().getLightFor(LightType.BLOCK, stock.getPosition()) / 15f;
        RenderContext.lastLightY = stock.getEntityWorld().getWorld().getLightFor(LightType.SKY, stock.getPosition()) / 15f;
