...
public class World {
    ...
    public static void registerEvents() {
        ...
        CommonEvents.World.UNLOAD.subscribe(world -> getWorldMap(world).remove(world.getDimension().getType().getRawId()));
    ...
    private Iterable<net.minecraft.entity.Entity> clientEntities() {
        return internal instanceof ClientWorld ? ((ClientWorld) internal).getEntities() : ((ServerWorld) internal).getEntities(null, e -> true);
    }

    @Environment(EnvType.SERVER)
    private Iterable<net.minecraft.entity.Entity> serverEntities() {
        return ((ServerWorld) internal).getEntities(null, e -> true);
    }

    private void checkLoadedEntities() {
        Iterable<net.minecraft.entity.Entity> internalEntities = ModCore.runOn(
                () -> this::clientEntities,
                () -> this::serverEntities
        );
    ...
    public Vec3i getPrecipitationHeight(Vec3i pos) {
        return new Vec3i(internal.getTopPosition(Heightmap.Type.MOTION_BLOCKING, pos.internal()));
    }
    ...
    public List<Vec3i> blocksInBounds(IBoundingBox bb) {
        return internal.method_20812(null, BoundingBox.from(bb))
                .map(VoxelShape::getBoundingBox)
                .filter(blockBox -> bb.intersects(IBoundingBox.from(blockBox)))
                .map(blockBox -> new Vec3i(blockBox.x1, blockBox.y1, blockBox.z1))
                .collect(Collectors.toList());
    }
    ...
    public void setBlock(Vec3i pos, ItemStack stack) {
        BlockState state = Block.getBlockFromItem(stack.internal.getItem()).getDefaultState();
    ...
    public int getRedstone(Vec3i pos) {
        ...
        for (Facing facing : Facing.values()) {
            power = Math.max(power, internal.getEmittedRedstonePower(pos.offset(facing).internal(), facing.internal));
        }
        return power;
    }
    ...
    public ItemStack getItemStack(Vec3i pos) {
        ...
        return new ItemStack(new net.minecraft.item.ItemStack(state.getBlock().asItem()));
    }

    /** Get dropped items within the given area */
    public List<ItemStack> getDroppedItems(IBoundingBox bb) {
        List<ItemEntity> items = internal.getEntities(ItemEntity.class, BoundingBox.from(bb), null);
