...
public class World {
    ...
    public BlockEntity reconstituteBlockEntity(TagCompound datain) {
        ...
        TileEntity te = (TileEntity) TileEntity.loadStatic(Blocks.AIR.defaultBlockState(), data.internal);
        if (te == null) {
    ...
    public Vec3i getPrecipitationHeight(Vec3i pos) {
        return new Vec3i(internal.getHeightmapPos(Heightmap.Type.MOTION_BLOCKING, pos.internal()));
    }
    ...
    public List<ItemStack> getDroppedItems(IBoundingBox bb) {
        List<net.minecraft.entity.item.ItemEntity> items =
                internal.getEntitiesOfClass(net.minecraft.entity.item.ItemEntity.class, BoundingBox.from(bb));
