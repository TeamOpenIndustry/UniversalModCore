...
public class ModdedEntity extends Entity implements IAdditionalSpawnData {
    ...
    private final SingleCache<IBoundingBox, Box> cachedCollisionBB = new SingleCache<>(BoundingBox::from);
    private final SingleCache<IBoundingBox, Box> cachedRenderBB = new SingleCache<>(internal -> {
        Box bb = BoundingBox.from(internal);
        /*
    ...
    public final boolean damage(DamageSource damagesource, float amount) {
        cam72cam.mod.entity.Entity wrapEnt = damagesource.getAttacker() != null ? self.getWorld().getEntity(damagesource.getAttacker()) : null;
        DamageType type;
    ...
    void removeSeat(SeatEntity seat) {
        ...
        if (passenger != null) {
            ...
            if (offset != null) {
                ...
                while (!(world.isAir(new Vec3i(pos).internal()) && world.isAir(new Vec3i(pos).up().internal()))) {
                    pos = pos.add(0, 1, 0);
                }
                passenger.setPosition(pos);
            }
    ...
    */
    ...
    @Override
    public Text getName() {
        return new LiteralText(this.type);
    }
     */
