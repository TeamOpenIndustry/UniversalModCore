...
public class ModCoreCommand extends Command {
	...
	private boolean sendChunkInfo(World world, Consumer<PlayerMessage> sender, Optional<Player> player, List<String> args) {
		...
		((ServerLevel) world.internal).getEntities().getAll().forEach(entities::add);
		...
		for (LevelChunk chunk : chunks) {
			...
			if (all || isChunkLocation || !hasChunkLocation && (teCount > 0 || entityCount > 0)) {
				...
				if (debug) {
					...
					for (net.minecraft.world.level.block.entity.BlockEntity tile : chunk.getBlockEntities().values()) {
					...
					counts.clear();
					for (net.minecraft.world.entity.Entity entity : chunkEntities) {
