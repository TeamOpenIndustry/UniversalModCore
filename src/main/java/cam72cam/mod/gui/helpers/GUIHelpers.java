...
public class GUIHelpers {
    ...
    public static void drawString(String text, int x, int y, int color, Matrix4 matrix4) {
        RenderState state = new RenderState().color(1, 1,1 ,1).alpha_test(true);
        state.model_view().multiply(matrix4);
        try (With ctx = RenderContext.apply(state)) {
            Minecraft.getInstance().font.draw(new MatrixStack(), text, (float) x, (float) y, color);
        }
    }
