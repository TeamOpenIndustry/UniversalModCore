...
public class ClientContainerBuilder extends AbstractContainerScreen<ServerContainerBuilder> implements IContainerBuilder {
    ...
    public int drawTopBar(int x, int y, int slots) {
        /*try (With ctx = RenderContext.apply(CHEST_TEXTURE))*/ {
            graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE.internal, centerX + x, centerY + y, 0, 0, paddingLeft, topOffset, 256, 256);
            // Top Bar
            for (int k = 1; k <= slots; k++) {
                graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE.internal, centerX + x + paddingLeft + (k - 1) * slotSize, centerY + y, paddingLeft, 0, slotSize, topOffset, 256, 256);
            }
            // Top Right Corner
            graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE.internal, centerX + x + paddingLeft + slots * slotSize, centerY + y, paddingLeft + stdUiHorizSlots * slotSize, 0, paddingRight, topOffset, 256, 256);
        }
        return y + topOffset;
    }
    ...
    public void drawSlot(ItemStackHandler handler, int slotID, int x, int y) {
        /*try (With ctx = RenderContext.apply(CHEST_TEXTURE))*/ {
            ...
            if (handler != null && handler.getSlotCount() > slotID) {
                graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE.internal, centerX + x, centerY + y, paddingLeft, topOffset, slotSize, slotSize, 256, 256);
            } else {
    ...
    public int drawSlotRow(ItemStackHandler handler, int start, int cols, int x, int y) {
        /*try (With ctx = RenderContext.apply(CHEST_TEXTURE))*/ {
            // Left Side
            graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE.internal, centerX + x, centerY + y, 0, topOffset, paddingLeft, slotSize, 256, 256);
            // Middle Slots
        ...
        /*try (With ctx = RenderContext.apply(CHEST_TEXTURE))*/ {
            // Right Side
            graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE.internal, centerX + x + paddingLeft + cols * slotSize, centerY + y, paddingLeft + stdUiHorizSlots * slotSize, topOffset, paddingRight, slotSize, 256, 256);
        }
        return y + slotSize;
    }
    ...
    public int drawBottomBar(int x, int y, int slots) {
        /*try (With ctx = RenderContext.apply(CHEST_TEXTURE))*/ {
            // Left Bottom
            graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE.internal, centerX + x, centerY + y, 0, textureHeight - bottomOffset, paddingLeft, bottomOffset, 256, 256);
            // Middle Bottom
            for (int k = 1; k <= slots; k++) {
                graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE.internal, centerX + x + paddingLeft + (k - 1) * slotSize, centerY + y, paddingLeft, textureHeight - bottomOffset, slotSize, bottomOffset, 256, 256);
            }
            // Right Bottom
            graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE.internal, centerX + x + paddingLeft + slots * slotSize, centerY + y, paddingLeft + 9 * slotSize, textureHeight - bottomOffset, paddingRight, bottomOffset, 256, 256);
        }
    ...
    public int drawPlayerTopBar(int x, int y) {
        /*try (With ctx = RenderContext.apply(CHEST_TEXTURE))*/ {
            graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE.internal, centerX + x, centerY + y, 0, 0, playerXSize, bottomOffset, 256, 256);
        }
        return y + bottomOffset;
    }
    ...
    public int drawPlayerMidBar(int x, int y) {
        /*try (With ctx = RenderContext.apply(CHEST_TEXTURE))*/ {
            graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE.internal, centerX + x, centerY + y, 0, midBarOffset, playerXSize, midBarHeight, 256, 256);
        }
    ...
    public int drawPlayerInventory(int y, int horizSlots) {
        ...
        /*try (With ctx = RenderContext.apply(CHEST_TEXTURE))*/ {
            graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE.internal, centerX + normInvOffset, centerY + y, 0, 126 + 4, playerXSize, 96, 256, 256);
        }
    ...
    public void drawSlotOverlay(ItemStack stack, int x, int y) {
        ...
        graphics.fill(RenderPipelines.GUI_TEXTURED, x, y, x + 16, y + 16, 0x80FFFFFF);
    ...
    public void drawSlotOverlay(String spriteId, int x, int y, double height, int color) {
        ...
        RenderSystem.setShaderTexture(0, minecraft.getTextureManager().getTexture(TextureAtlas.LOCATION_BLOCKS).getTextureView());
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, x, y, 0, 16, 16);
    }
